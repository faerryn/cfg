...
#+begin_src emacs-lisp
  ...
  (let ((async-shell-command-regexp "\\`\\*Async Shell Command\\*\\(?:<[[:digit:]]+>\\)?\\'"))
...
#+begin_src emacs-lisp
  ...
  (with-eval-after-load "exwm"
    (custom-set-variables '(exwm-mode-map nil))
    (defun faerryn/exwm-buffer-name ()
      (exwm-workspace-rename-buffer (concat "*EXWM* " (or exwm-title exwm-class-name))))
    (add-hook 'exwm-update-class-hook #'faerryn/exwm-buffer-name)
    (add-hook 'exwm-update-title-hook #'faerryn/exwm-buffer-name))
#+end_src
* Follow Symlinks
#+begin_src emacs-lisp
  (custom-set-variables
