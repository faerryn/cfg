...
#+end_src
** Rainbow
Paint the background of color codes with the color they represent.
#+begin_src emacs-lisp
...
#+begin_src emacs-lisp
  (defun faerryn/exwm-toggle-input ()
    "Toggle EXWM buffers between `line-mode' and `char-mode'."
    (interactive)
  ...
  (with-eval-after-load "exwm"
    (defcustom faerryn/exwm-input-global-keys nil
      "Key chords that are passed to Emacs while in  `char-mode'."
    ...
    (define-key exwm-mode-map (kbd "C-M-<delete>") #'faerryn/exwm-toggle-input))
  (custom-set-variables
...
#+begin_src emacs-lisp
  (with-eval-after-load "exwm"
    (defun faerryn/nil (&rest ignored))
    (defun faerryn/exwm-workspace-init-fix (oldfun)
      (advice-add #'modify-all-frames-parameters :override #'faerryn/nil)
      (funcall oldfun)
      (advice-remove #'modify-all-frames-parameters #'faerryn/nil))
...
This edits the EXWM fullscreen behavior to be nicer.
#+begin_src emacs-lisp
...
#+end_src
** Buffer Predicate
This filters what buffers [[help:other-buffer]] will show.
#+begin_src emacs-lisp
  (defun faerryn/buffer-predicate (buf)
    (or (buffer-file-name buf)
        (with-current-buffer buf
          (derived-mode-p 'lisp-interaction-mode 'eshell-mode 'exwm-mode))))
  (modify-all-frames-parameters '((buffer-predicate . faerryn/buffer-predicate)))
#+end_src
** Compiler Explorer
...
#+end_src
** Git Client
Magit is a very nice git client for Emacs.  The killer feature (for
me) is staging small chunks at a time to split up big changes into
multiple commits.
#+begin_src emacs-lisp
