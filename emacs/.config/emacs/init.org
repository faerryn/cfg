    ...
    (add-hook
     ...
     (lambda ()
       (call-process "xrdb" nil nil nil "-merge"
                     (expand-file-name "Xresources" (getenv "XDG_CONFIG_HOME")))
       (make-process :name "sxhkd" :command '("sxhkd") :noquery t)
       (make-process :name "picom" :command '("picom") :noquery t)
       (make-process :name "pulseaudio"
                     :command '("pulseaudio" "--exit-idle-time" "-1") :noquery t)
...
#+begin_src emacs-lisp
  (use-package vterm
    :commands vterm
    :init (setq vterm-environment '("COLORTERM=truecolor")))
#+end_src
