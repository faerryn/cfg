...
#+begin_src emacs-lisp
  (setq user-emacs-directory
        (expand-file-name
         "emacs/"
         (or (getenv "XDG_DATA_HOME") (getenv "APPDATA"))))
#+end_src
** Visible Bell
#+begin_src emacs-lisp
  (setq visible-bell t)
...
#+begin_src emacs-lisp
  (use-package doom-themes
    :hook (emacs-startup-hook . (lambda () (load-theme 'doom-one t))))
#+end_src
** Evil
...
#+begin_src emacs-lisp
  (use-package exwm
    ...
    :init
    (setq exwm-input-prefix-keys '(?\C-x ?\C-h ?\C-g ?\M-x ?\M-&))
    (setq exwm-mode-map nil)
    :config
    (add-hook 'exwm-init-hook
              (lambda () (make-process :name "sxhkd" :command '("sxhkd") :noquery t)))
    (add-hook 'exwm-floating-setup-hook
              (lambda () (set-window-dedicated-p (frame-selected-window) nil)))
